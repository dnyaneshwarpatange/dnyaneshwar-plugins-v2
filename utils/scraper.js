...
async function checkCompatibility(plugins, targetDCVersion, progressCallback) {
  ...
  const browser = await puppeteer.launch({
    headless: true,                    // 'new' mode has crashpad issues on Linux VPS — use true
    executablePath: process.env.PUPPETEER_EXECUTABLE_PATH || undefined,
    args: [
      '--no-sandbox',                  // Required on Linux — no user namespace support on most VPS
      '--disable-setuid-sandbox',      // Same as above
      '--disable-dev-shm-usage',       // VPS /dev/shm is often tiny (64MB) — use /tmp instead
      '--no-crashpad',                 // ← KEY FIX: disables chrome_crashpad_handler entirely
      '--disable-crash-reporter',      // Belt-and-suspenders: also kills crash reporter
      '--disable-gpu',                 // No GPU on VPS headless environment
      '--disable-software-rasterizer', // Prevents GPU fallback crash
      '--disable-extensions',          // Fewer moving parts = more stable
      '--no-first-run',                // Skip Chrome first-run setup
      '--no-zygote',                   // Fixes crashes on some Linux kernels
      '--single-process',              // Last resort for very restricted VPS (use only if above still crashes)
    ]
  });
